...
import { NvPageHead, NvSeg, NvStat, NvStatRow, NvEmpty } from "../components/atoms";
...
export default function Risk() {
  ...
  const [claimStatus, setClaimStatus] = useState("loading"); // loading|ok|empty|error
  const [fb, setFb] = useState({ penalty: true });
  ...
  const loadClaims = async () => {
    ...
    try {
      ...
      if (mapped.length) { setClaims(mapped); setClaimStatus("ok"); }
      else { setClaims([]); setClaimStatus("empty"); }
    } catch { setClaims(FALLBACK_CLAIMS); setClaimStatus("error"); }
    setLoading(false);
  };
  ...
  // 위험 항목 — penalty 스캔 실데이터 주입 (danger가 배열/숫자 어느 쪽이든 안전 처리)
  const usagePct = penalty?.usagePercent != null ? penalty.usagePercent
    : (penalty && penalty.total != null ? Math.round((penalty.total / (penalty.limit || 1000)) * 1000) / 10 : 0);
  const dangerRaw = penalty?.danger;
  const idleCount = Array.isArray(dangerRaw) ? dangerRaw.length : (typeof dangerRaw === "number" ? dangerRaw : 0);
  const limitCount = (penalty && penalty.limit && penalty.total >= penalty.limit * 0.95) ? 1 : 0;
  const risks = [
    ...
    { key: "idle", icon: "flame", title: "무거래 상품", desc: "13개월 무판매 상품 → 삭제 권고 (네이버 14개월 정리 대비)", count: idleCount, action: "삭제 권고 보기", linked: "페널티 스캔 연동", tone: "amber" },
    { key: "limit", icon: "list", title: "등록 한도 임박", desc: penalty ? `${penalty.total} / ${penalty.limit || "—"}개 · ${usagePct}% 사용` : "스캔 후 표시", count: limitCount, action: "상품 정리", linked: "페널티 스캔 연동", tone: "green" },
  ];
  const weights = { claim: 35, idle: 25, limit: 0 };
    ...
    <>
      ...
      {tab === "claims" && (
        <>
          ...
          {claimStatus === "error" && <div style={{ marginBottom: 12 }}><div className="nv-banner amber"><span className="bi"><NvIcon name="bell" size={17} /></span><span>클레임 데이터를 불러오지 못해 예시를 표시합니다. 클레임 폴링을 눌러 주세요.</span></div></div>}
          {claimStatus === "empty" ? (
            <NvEmpty icon="check" title="처리할 클레임이 없어요" desc="최근 들어온 반품·취소·교환 클레임이 없습니다. 새 클레임이 생기면 AI가 자동 분류해 여기에 표시돼요." />
          ) : claimStatus === "loading" ? (
      ...
      {tab === "penalty" && (
        <>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 32, flexWrap: "wrap" }}>
              ...
              <div style={{ flex: 1, minWidth: 240 }}>
                ...
                <div style={{ display: "flex", gap: 28 }}>
                  {[[handled, "안전 항목", "var(--ink)"], [pending, "처리 필요", sCol], [approved, "클레임 자동 승인", "var(--green-ink)"]].map((x, i) => (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 16 }}>
            <div className="nv-card">
              <div className="nv-card-h"><h3 className="nv-card-t" style={{ color: "var(--amber-ink)", display: "flex", alignItems: "center", gap: 6 }}><NvIcon name="flame" size={16} /> 무거래 상품</h3><span className="nv-card-hint" style={{ color: "var(--amber-ink)" }}>13개월+</span></div>
              {!penalty ? (
                <div style={{ padding: "28px 0", textAlign: "center", color: "var(--ink-3)", fontSize: 13 }}>위험 재스캔을 누르면 표시됩니다.</div>
              ) : idleCount === 0 ? (
                <div style={{ padding: "28px 0", textAlign: "center", color: "var(--ink-3)", fontSize: 13 }}>13개월 이상 무거래 상품이 없어요.</div>
              ) : (
                <div style={{ padding: "18px 0", textAlign: "center" }}>
                  <div className="mono" style={{ fontSize: 40, fontWeight: 800, color: "var(--amber-ink)", lineHeight: 1 }}>{idleCount}</div>
                  <div style={{ fontSize: 13, color: "var(--ink-2)", marginTop: 8, lineHeight: 1.5 }}>13개월 이상 판매가 없는 상품이에요.<br />네이버 14개월 무거래 정리 대상이 되기 전에<br />정리하거나 가격·노출을 점검하세요.</div>
                  <button className="nv-btn ghost" style={{ marginTop: 16, justifyContent: "center" }}><NvIcon name="list" size={13} /> 무거래 상품 목록 보기</button>
                </div>
              )}
            </div>
            <div className="nv-card">
              <div className="nv-card-h"><h3 className="nv-card-t">클레임 처리 현황</h3><span className="nv-card-hint">이번 조회 기준</span></div>
              {claims.length === 0 ? (
                <div style={{ padding: "28px 0", textAlign: "center", color: "var(--ink-3)", fontSize: 13 }}>처리한 클레임이 없어요.</div>
              ) : [["AI 자동 승인 완료", approved], ["셀러 검토 대기", held], ["전체 분류 건수", claims.length]].map((x, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13, padding: "11px 0", borderBottom: i < 2 ? "1px solid var(--line-2)" : "none" }}>
